...
import { Paper, Typography, Stack, Chip, keyframes } from "@mui/material";
...
const ConnectionMonitor = () => {
  ...
  useEffect(() => {
    ...
    // If no userId is found, generate a new one and store it
    if (!userId) {
      userId = "user-" + Math.random().toString(36).substr(2, 9);
      localStorage.setItem("userId", userId);
    }

    // Use the userId to construct the WebSocket URL
    ...
    <StatusPaper connectionStatus={status}>
      ...
      >
        <Stack direction="row" spacing={1} alignItems="center">
          {status === "connected" ? (
            <CheckCircleIcon
              sx={{ color: "var(--success-color)", fontSize: 20 }}
            />
          ) : (
      ...
      {error && <ErrorText sx={{ mt: 0.5 }}>{error}</ErrorText>}
    </StatusPaper>
  );
};
